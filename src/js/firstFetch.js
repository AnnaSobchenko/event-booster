...
async function startPage(idCountry, countryName) {
  ...
  try {
    ...
    //     `https://app.ticketmaster.com/discovery/v2/events.json?size=21&countryCode=${idCountry}&source=Ticketmaster&apikey=iy4jLWk8h3FEKwR9yFh4EdYPL6zmAm9o`,
    //   );
    //   const finalEl = await res.json();
    //   renderCardForTablet(finalEl, countryName);
    // } else {
    const res = await fetch(
      `https://app.ticketmaster.com/discovery/v2/events.json?countryCode=${idCountry}&source=Ticketmaster&apikey=iy4jLWk8h3FEKwR9yFh4EdYPL6zmAm9o`,
    );
    const finalEl = await res.json();
    renderCardForTablet(finalEl, countryName);
    // }
